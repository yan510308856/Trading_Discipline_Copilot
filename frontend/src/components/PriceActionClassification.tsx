...
export function PriceActionClassification({ trade, compact = false, englishOnly = false }: { trade: Trade; compact?: boolean; englishOnly?: boolean }) {
  ...
  return <section className="price-action-classification"><div className="classification-heading"><strong>Price Action Classification{!englishOnly && <> / <span lang="zh">价格行为分类</span></>}</strong>{classification.legacy && <span>{englishOnly ? "Legacy classification" : "Legacy classification / 历史分类"}</span>}</div><div className="classification-values"><Value label="Market State" chinese="市场结构" item={state} englishOnly={englishOnly} /><Value label="Trade Thesis" chinese="交易逻辑" item={thesis} englishOnly={englishOnly} /><Value label="Entry Trigger" chinese="入场触发" item={trigger} englishOnly={englishOnly} /><div className="classification-locations"><span>Key Locations{!englishOnly && <> / <span lang="zh">关键位置</span></>}</span>{classification.locationTags.length ? <div>{classification.locationTags.map((value) => { const item = findTaxonomyItem(locationTagOptions, value); return item && <span key={value}>✓ {item.english}{!englishOnly && <> / <span lang="zh">{item.chinese}</span></>}</span>; })}</div> : <small>No key locations{!englishOnly && " / 未标记关键位置"}</small>}</div></div></section>;
}
